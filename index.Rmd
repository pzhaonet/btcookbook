...
barplot_car <- barplot(df.o$mpg, border = df.o$bd, col = df.o$cyl, ylab = 'mpg', main = 'Miles per gallon')
legend('topleft', legend = unique(df.o$cyl), fill = unique(df.o$cyl), title = 'cyl', border = NA, bty = 'n')
text(x = barplot_car[cl_red], y = df.o$mpg[cl_red], labels = df.o$hp[cl_red])
mtext('car', 1, 8)
axis(1, at = barplot_car, labels = df.o$car, las = 2)
```
...
df.sort %>% 
  ...
  geom_col()+
  geom_col(data= df.sort[cl_red,], color="red", size=1)+
  geom_text(data= df.sort[cl_red,], aes(label=hp))+
