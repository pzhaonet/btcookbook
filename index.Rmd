...
legend('topright', legend = 1:4, col = 1:4, 
       lty = 1, title = 'z')

# or
for (z in 4:1) 
  curve(z/x, 0, 10, add = z < 4, col = z)
legend('topright', legend = 4:1, col = 4:1, 
       lty = 1, title = 'z')

# or
matplot(outer(seq(0.1, 10, by = 0.1), 1:4, 
              function(x, a) a / x),
...
library(ggplot2)
library(magrittr)
expand.grid(x = seq(0.1, 10, by = 0.1),
            z = 1:4) %>% 
...
for (z in 1:4) {
  out = out + 
    stat_function(fun = function(.x) z/.x, 
                  color = z)
}
out
```
